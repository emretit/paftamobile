-- Simple multi-tenant setup without foreign key issues
-- Step 1: Create tables with minimal constraints
CREATE TABLE IF NOT EXISTS public.user_prefs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,  -- Remove foreign key constraint to avoid issues
  current_org_id uuid,
...
-- Create policies that work without foreign keys
...
-- Create a default organization for the current authenticated user
-- Using the user ID we know exists: eacc3cf1-b059-4578-bbac-f1429b3a10d0
DO $$
DECLARE
...
BEGIN
  ...
  -- Create org membership (use ON CONFLICT to avoid duplicates)
